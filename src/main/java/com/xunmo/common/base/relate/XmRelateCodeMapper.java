    ...
    default T getParentByChildCode(String childDeptCode){
        ...
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            if (!configuration.hasStatement(id, false)) {
                ...
                String sql = StrUtil.format("select * from {tableName} where code = (select parent_code from {tableName} where code = #{code})", columnNameMap);
                RawSqlSource rawSqlSource = new RawSqlSource(configuration, sql, Object.class);
                MappedStatement.Builder builder = new MappedStatement.Builder(configuration, id, rawSqlSource, SqlCommandType.SELECT);

                // 创建返回映射
                ArrayList<ResultMap> resultMaps = new ArrayList<>();
                ResultMap.Builder resultMapBuilder = new ResultMap.Builder(configuration, UUID.fastUUID().toString(true), modelClass, new ArrayList<>());
    ...
    default List<T> getAllParentNotSelfById(String deptId) {
        ...
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            if (!configuration.hasStatement(id, false)) {
                ...
                String sql = StrUtil.format("select * from {tableName} where code in (\n" +
    ...
    default List<T> getAllChildNotSelfByIds(List<String> deptIds){
        ...
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            if (!configuration.hasStatement(id, false)) {
                ...
                String sql = StrUtil.format("<script>select * from {tableName} where code in (\n" +
                        "        SELECT child_code FROM {tableNameRelate} WHERE 1=1\n" +
                        "        and disabled = 0\n" +
